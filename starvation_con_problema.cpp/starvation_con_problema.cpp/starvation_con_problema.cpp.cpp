...
public:
    ...
    void run() {
        ...
        // detenemos consumidores ahora mismo, dejando tareas en la cola.
        stop_consumers = true;
        ...
        int pendingTotal = 0;
        {
            ...
            for (const auto& t : queue_) {
                if (t.type == 'B') ++pendingBfinal;
            }
        }

        std::cout << "\n=============================\n";
        std::cout << "Resumen final (CON starvation)\n";
        ...
        std::cout << "Tareas B en espera al final: " << pendingBfinal << "\n";
        std::cout << "Tiempo de espera maximo observado para una tarea B: "
            << max_wait_B_ms.load() << " ms\n";
        std::cout << "=============================\n\n";
    }
...
private:
    ...
    std::atomic<long long> max_wait_B_ms{ 0 };
    ...
    // Selección de tarea sin aging: siempre A, luego M, luego B
    size_t select_task_index_unlocked() {
        ...
        if (idxM != -1) return (size_t)idxM;

        // Si no hay ni A ni M, tomamos la primera (será B)
        return 0;
    }
    ...
    void consumer(int consumerId) {
        ...
        while (true) {
            ...
            auto wait_ms = duration_cast<milliseconds>(now - task.enqueue_time).count();

            // Máximo tiempo de espera de B
            if (task.type == 'B') {
                long long current = max_wait_B_ms.load();
                while (wait_ms > current &&
                    !max_wait_B_ms.compare_exchange_weak(current, wait_ms)) {
                    // retry
                }
            }
    ...
    void monitor() {
        std::cout << "\n=====================================\n";
        std::cout << "VERSION CON STARVATION\n";
